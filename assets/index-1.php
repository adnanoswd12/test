...
<!--[if gt IE 8]><!--> <html class="no-js" lang="en"> <!--<![endif]-->
  <head>
    ...
    <link rel="stylesheet" href="assets/css/foundation.css?v=11">
...
<link rel="stylesheet" href="assets/css/app-events.css?v=6">
...
<!-- Top Right Menu -->
            ...
            <!-- Logo -->
            <div class="four left mobile-four"><br>

                <a href="https://thechangeleadership.com/" title="The Change Leadership Conference"><img src="assets/images/Change-Leadership-Global-Logo.png" alt="The Change Leadership Conference" height="36" class="logo"></a>
            </div>
        </div>
    </header>
...
<!--        <div class="article-title bg-marketing">
            <h1 style="color:#fff; padding: 0px; margin: 0em .5em; font-weight: normal; font-size: 36px; padding: 0.5em 0 0.2em 0;">Register for The Change Leadership Conference</h3>
                    ...
                    <div class="price-break-message" style="display: inline-block; padding: 20px 20px 20px 90px; margin-bottom: 20px; border: 3px dashed #003366; background: #EAF2FB url('assets/images/bulk.png') no-repeat; background-position: 10px center; width:100%; font-size: 16px;">
...
<br>
    ...
    background-position: right center;
        ...
        font-size:24px;
    color: #FF7F10;
    cursor: pointer;" type="submit" value="REGISTER NOW" name="form_submit">
        ...
        $.post('process.php',{"a_couponName":couponName},function(data){  
		 ...
		 if(data == 'error'){
		  ...
		  if('percentage' == data[0]){
			...
			var subtotaal = (subtotal-(subtotal/100)*data[1]);
		    $('#calc-subtotal').html(subtotaal);
			t = subtotaal + totalTax;
			unsaved = subtotaal;
		    $('#calc-total').html(t);
			$('#calc-savings').html('$'+(subtotal/100)*data[1]);
		   ...
		   if('number' == data[0]){
			   if(tickets < <?php echo json_encode($bulk_amt)?>){
		   subtotal = subtotal-data[1];
		   
	       $('#calc-subtotal').html(subtotal);
		   var t = subtotal + totalTax;
		   $('#calc-total').html(t);
		   $('#calc-savings').html('$'+data[1]);
			   }
		   $('#calc-total').formatCurrency();
		   $('#calc-subtotal').formatCurrency();
		     $('#calc-savings').formatCurrency();
