...
<!--        <div class="article-title bg-marketing">
        ...
        <?php if(isset($_REQUEST['form_submit'])){ ?>
        <div class="error-msg flash-warning" style="display:block">
            <span id="showerror">
			</span>
           </div>
			<?php 	}
      ...
      <input type="hidden" id="saving" name="data[saving]">
            ...
            <div class="eight columns pull-four mobile-2 form-fields">
                <div class="input radio"><strong>Salutation <em>*</em></strong><input type="radio" name="data[salutation]" id="SalutationMrs" value="Mrs." checked="checked"  /><label for="SalutationMrs">Mrs.</label><input type="radio" name="data[salutation]" id="SalutationMs" value="Ms."  /><label for="SalutationMs">Ms.</label><input type="radio" name="data[salutation]" id="SalutationMiss" value="Miss"  /><label for="SalutationMiss">Miss</label><input type="radio" name="data[salutation]" id="SalutationMr" value="Mr."  /><label for="SalutationMr">Mr.</label></div>                <div style="clear:left;"></div>
                 <div class="input text"><label for="first_name">First Name <em>*</em></label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['first_name'];}?>" name="data[first_name]" type="text" class="required" id="first_name" /></div><div class="input text"><label for="last_name">Last Name <em>*</em></label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['last_name'];}?>" name="data[last_name]" type="text" class="required" id="last_name" /></div><div class="input text">
                
                <label for="company">Company <em>*</em></label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['company'];}?>"  name="data[company]" type="text" class="required" id="company" /></div><div class="input select"><label for="industry">Industry <em>*</em></label><select value="<?php if(isset($_REQUEST['form_submit'])){echo $d['industry'];}?>" name="data[industry]" class="required" id="industry">
...
</select></div><div class="input text"><label for="title">Job Title <em>*</em></label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['title'];}?>"  name="data[title]" type="text" class="required" id="title" /></div><div class="input text"><label for="phone">Phone <em>*</em></label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['phone'];}?>" name="data[phone]" type="text" class="required" id="phone" />Ext: <input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['extension'];}?>"   name="data[extension]" type="text" size="3" id="extension" /></div><div class="input text"><label for="email">Email <em>*</em></label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['email'];}?>"  name="data[email]" type="text" class="required email" id="email" /></div><div class="input text"><label for="address_1">Address <em>*</em></label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['address_1'];}?>"  name="data[address_1]" type="text" class="required" id="address_1" /></div><div class="input text"><label for="address_2">Unit / Suite #</label><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['address_2'];}?>" name="data[address_2]" type="text" id="address_2" /></div>
<div class="input text"><label for="city">City <em>*</em></label><input  value="<?php if(isset($_REQUEST['form_submit'])){ echo $d['city'];}?> "  name="data[city]" type="text" class="required" id="city" /></div><div class="input select"><label for="province">Province <em>*</em></label>
 <select  value="<?php if(isset($_REQUEST['form_submit'])){echo $d['province'];}?>"  name="data[province]" class="required" id="province">
<option value="">Please Select</option>
...
</select></div><div class="input text"><label for="postal_code">Postal Code <em>*</em></label><input  value="<?php if(isset($_REQUEST['form_submit'])){echo $d['postal_code'];}?>" name="data[postal_code]" type="text" class="required" id="postal_code" /></div><div class="input text"><label for="country">Country <em>*</em></label><input   name="data[country]" type="text" class="required" value="Canada" id="country" /></div><div class="input select"><label for="heard_about">How did you hear about us? <em>*</em></label>
<select  value="<?php if(isset($_REQUEST['form_submit'])){echo $d['heard_about'];}?> "  name="data[heard_about]" class="required" id="heard_about">
<option value="">Please Select</option>
...
</select><input value="<?php if(isset($_REQUEST['form_submit'])){echo $d['heard_about_other'];}?>"  name="data[heard_about_other]" type="text" style="display:none; width: 360px;" placeholder="Tell us where?" size="3" id="heard_about_other" /></div>
            </div>
        </div>
...
<?php if(isset($_REQUEST['form_submit'])){
	...
	?>
    <script>
    $("#industry").val(<?php echo json_encode($d['industry'])?>);
	 $("#heard_about").val(<?php echo json_encode($d['heard_about'])?>);
	 $("#province").val(<?php echo json_encode($d['province'])?>);
    // $("#gen_quantity").val(<?php echo json_encode($d['gen_quantity'])?>);
	// $("#vip_quantity").val(<?php echo json_encode($d['vip_quantity'])?>);
	// $("#pla_quantity").val(<?php echo json_encode($d['pla_quantity'])?>);
</script>
<?php } ?>
            </section>
...
$amount = urlencode(number_format($totalAmount,2));
$currencyCode="CAD";
...
if(!empty($resArray['L_LONGMESSAGE0'])){
	//header("Location: https://thechangeleadership.com/register?error=".$resArray['L_LONGMESSAGE0']);
    $error = $resArray['L_LONGMESSAGE0'];
	//echo "<input type='hidden' name='data[paypalerror]' value='".$resArray['L_LONGMESSAGE0']."'>";
	?>
	<script>
    $("#showerror").html(<?php echo json_encode($error); ?>);
    </script>
...
while($data = mysqli_fetch_array($selectTicket)){
	//print_r($data);
	//echo '<br><br><br>';
	$ticket_user_id  = $data['user_id'];
	}
//print_r($getTicket);
$ticketID = '100'.$ticket_user_id;
//echo $ticketID;
...
$message = '<h3><strong>Registration Information</strong>,</h3><p><strong>First Name</strong>:<br>'.$first_name.';</p><p><strong>Last Name</strong>:<br>'.$last_name.'</p><p><strong>Email Address</strong>:<br>'.$email.'</p><p><strong>Job Title</strong>:<br>'.$job_title.'</p><p><strong>Company Name</strong>:<br>'.$company.'</p><p><strong>Industry</strong>:<br>'.$industry.'</p><p><strong>Address</strong>:<br>'.$address_1.'</p><p><strong>City</strong>:<br>'.$city.'</p><p><strong>Province</strong>:<br>'.$province.'</p><p><strong>Postal Code</strong>:<br>'.$postal_code.'</p><p><strong>Country</strong>:<br>'.$country.'</p><p><strong>Phone</strong>:<br>'.$phone.'</p><p><strong>How did you hear about us?</strong><br>'.$how_did_you_hear.'</p><p><strong>Participant Name (s):</strong> '.$stringParticipant.'<br><strong>Amount Paid:</strong> $'.$amount.'<br><strong>General Quantity :</strong> '.$gen_quantity.'<br><strong>Professional Quantity :</strong> '.$vip_quantity.'<br><strong>VIP Quantity :</strong> '.$pla_quantity.'<br><br><strong>Order Number:</strong> '.$transactionId.'<br><strong>Ticket Number:</strong> '.$ticketID.'</p></p>';
