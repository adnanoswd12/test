...
<link rel="stylesheet" href="assets/css/app-events.css?v=4">
    ...
    <script src="assets/js/jquery.offcanvas.js"></script>
    <script src="assets/js/jquery.colorbox-min.js"></script>
    <script src="assets/js/jquery.validate.min.js"></script>
    <script src="assets/js/jquery.format-currency.min.js"></script>
...
//if(isset($_POST['a_couponName'])){
 ...
 echo json_encode($couponExists);
  ...
  //}
  $gData = mysql_query("SELECT * FROM `wp_form_settings` WHERE `id` = '1' ");
  //print_r($getData);
  $getData = mysql_fetch_array($gData);
 // print_r($row);
...
<!-- Top Right Menu -->
    ...
    <section id="sidebar" role="complementary">
        ...
        <div class="padding">
            <p class="date marketing" style="margin: 0;">
                                    May  31, 2017                            </p>
            <p class="time" style="font-size: 13px; font-weight: normal; line-height: 15px; margin: 0 0 8px 0; text-align: center;">
                9:30am - 3:30pm            </p>
        </div>
        ...
        <div class="padding">
            <div data-link-location="/marketing-toronto-2017/details#where">
                <p class="date-loc">
                    <span class="venue" style="font-size: 18px; line-height: 22px;">Metro Toronto Convention Centre</span><br>
                </p>
            </div>
        ...
        <div class="padding" style="padding-top: 20px;">
                                                                        <h5 style="margin:0px;">General Pass</h5>
                        <div class="register-ticket-desc general-admission">
                            <p>- Session with John Maxwell on Leading with Influence<br>
- Session with Jeremy Gutsche on Disruptive Thinking <br>
- Tea and Coffee Included<br>
- Change Leadership Brochure &amp; Magazine<br>
- Book signing opportunities with speakers<p>                        </div>
                    
                                            <h5 style="margin:0px;">VIP Pass</h5>
                        <div class="register-ticket-desc vip-admission">
                            <p>Express entrance</p><p>Reserved seating directly behind Platinum</p><p>Latest issue of The Art of Magazine</p><p>Book signing opportunities with speakers</p><p>Exclusive three course networking lunch</P><p>An eco-friendly tote bag, notebook and pen</p><p>Copies of 3 featured bestselling books:</p><ul><li>Growth Hacker Marketing - Ryan Holiday</li><li>This I Know - Terry O'Reilly</li><li>Everyone's An Artist - Ron Tite</li></ul>                        </div>
                    
                                            <h5 style="margin:0px;">Platinum Pass</h5>
                        <div class="register-ticket-desc platinum-admission">
                            <p> <b>Meet and greet photo opportunity with Troy Carter</b> </p><p> <b>Continental breakfast and coaching session</b> </p> <p>Express entrance</p> <p>Reserved premier seating in the first row</p><p>Latest issue of The Art of Magazine</p> <p>Book signing opportunities with speakers<p> <p>Exclusive three course networking lunch<p><p>An eco-friendly tote bag, notebook and pen</p> <p>Copies of 5 featured bestselling books:</p><ul><li>Growth Hacker Marketing - Ryan Holiday</li><li>This I Know - Terry O'Reilly</li><li>Everyone's An Artist - Ron Tite</li><li>Invisible Influence - Jonah Berger</li> <li>Everybody Writes - Ann Handley</li></ul>                        </div>
                                                        </div>
    </div>
...
<!--        <div class="article-title bg-marketing">
        ...
        <div class="stripes marketing" style="margin-bottom: 1em;">&nbsp;</div>
                    ...
                    <div class="price-break-message" style="display: inline-block; padding: 20px 20px 20px 50px; margin-bottom: 20px; border: 2px solid #ffcc00; background: #fffccc url('assets/images/star_full.png') no-repeat; background-position: 10px center; width:100%;">
...
<?php 
   ...
   ?>
      ...
      <input type="hidden" id="bulk_gen_ticket" name="bulk_gen_ticket" value="<?php echo ($popt-$bulk_disc)?>" />
...
<script type="text/javascript">
    ...
    //function update_total() {
//
//        // pull out ticket quantities and prices
...
//        per_gen_ticket = ( tickets >= 5 ) ? $('#bulk_gen_ticket').val() : $('#per_gen_ticket').val();
//        per_vip_ticket = ( tickets >= 5 ) ? $('#bulk_vip_ticket').val() : $('#per_vip_ticket').val();
//        per_pla_ticket = ( tickets >= 5 ) ? $('#bulk_pla_ticket').val() : $('#per_pla_ticket').val();
//
//        // calculate sub-total / tax / total
//        $('#gen-price').html( per_gen_ticket );
//        $('#vip-price').html( per_vip_ticket );
//        $('#pla-price').html( per_pla_ticket );
...
//
//        // calculate savings
//        unsaved_total = ( gen_quantity * 449.00 ) + ( vip_quantity * 649.00 ) + ( pla_quantity * 799.00 );
//
//        // strike-through general price on discount
//        if( per_gen_ticket < 449.00 ) {
//            $('#gen-price').html( "<del style='color:red;'>$449.00</del> " + "<em style='color:green; font-style: normal;'>$" + per_gen_ticket + "</em>" );
//        } else {
//            $('#gen-price').html( '$' + parseFloat(per_gen_ticket).toFixed(2) );
//        }
//
//        // strike-through vip price on discount
//        if( per_vip_ticket < 649.00 ) {
//            $('#vip-price').html( "<del style='color:red;'>$649.00</del> " + "<em style='color:green; font-style: normal;'>$" + per_vip_ticket + "</em>" );
//        } else {
//            $('#vip-price').html( '$' + parseFloat(per_vip_ticket).toFixed(2) );
//        }
//
//        // strike-through platinum price on discount
//        if( per_pla_ticket < 799.00 ) {
//            $('#pla-price').html( "<del style='color:red;'>$799.00</del> " + "<em style='color:green; font-style: normal;'>$" + per_pla_ticket + "</em>" );
//        } else {
...
//
//        // make individual extra attendee fields required / not required
//        for( n=2; n<=20; n=n+1 ) {
//            if( tickets >= n ) {
//                $('#attendee-' + n).slideDown();
//                $('#attendee_' + n + '_first_name').removeClass('ignore');
//                $('#attendee_' + n + '_last_name').removeClass('ignore');
//                $('#attendee_' + n + '_company').removeClass('ignore');
//                $('#attendee_' + n + '_title').removeClass('ignore');
//                $('#attendee_' + n + '_email').removeClass('ignore');
//                $('#attendee_' + n + '_phone').removeClass('ignore');
//            } else {
//                $('#attendee_' + n + '_first_name').addClass('ignore');
//                $('#attendee_' + n + '_last_name').addClass('ignore');
//                $('#attendee_' + n + '_company').addClass('ignore');
//                $('#attendee_' + n + '_title').addClass('ignore');
//                $('#attendee_' + n + '_email').addClass('ignore');
//                $('#attendee_' + n + '_phone').addClass('ignore');
//
//                $('#attendee_' + n + '_first_name').removeClass('error');
//                $('#attendee_' + n + '_last_name').removeClass('error');
//                $('#attendee_' + n + '_company').removeClass('error');
//                $('#attendee_' + n + '_title').removeClass('error');
//                $('#attendee_' + n + '_email').removeClass('error');
//                $('#attendee_' + n + '_phone').removeClass('error');
//                $('#attendee-' + n).slideUp( );
//            }
//        }
//
//        // hide or show the entire extra-attendee box
//        if( tickets >= 2 ) {
//            $('#extra-attendees').slideDown();
//        } else {
//            $('#extra-attendees').slideUp( );
//        }
//    }
</script>
            </section>
...
if(isset($_POST['form_submit'])){
	...
	$getExistEmailQuery = mysql_query( "SELECT * FROM `wp_ticket_users` WHERE `email` = '$getEmail'" );
	$getExistEmail = mysql_fetch_array($getExistEmailQuery);
	...
	if(empty($getExistEmail)){
		...
		$insertTicket = mysql_query("INSERT INTO `wp_ticket_users` (`first_name`, `last_name`, `company`, `industry`, `job_title`, `phone`, `email`, `address_1`, `address_2`, `city`, `province`, `postal_code`, `country`, `how_did_you_hear`, `gen_quantity`, `vip_quantity`, `pla_quantity`, `totalTax`, `totalAmount`) VALUES ('$first_name','$last_name','$company','$industry'
		,'$job_title'
		,'$phone'
		,'$email'
		,'$address_1'
	 ...
	 for($i=0;$i<=($extraData['tickets']-1);$i++){
		...
		$metaInsert = mysql_query("INSERT INTO `wp_ticket_users_meta` ( `first_name`, `last_name`, `company`, `job_title`, `phone`, `email`, `user_email`) VALUES ( '$firstvarname', '$lastnamevar', '$companyvar', '$jobvar', '$phonevar', 
		'$evar',
		'$email')");
		if(!$metaInsert){
			echo mysql_error();
